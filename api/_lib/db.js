...
/**
 * Lazy accessor for the Drizzle db. Throws a clear error if Postgres is
 * disabled — protects against accidental queries during the dual-write
 * window.
 */
function getDb() {
  if (!isPostgresEnabled() && !isDualWriteEnabled()) {
    throw new Error(
      '[db] getDb() called but DB_BACKEND=sheets and DB_DUAL_WRITE is off. ' +
        'Wrap call sites in isPostgresEnabled() / isDualWriteEnabled() checks.'
    );
  }
...
async function dbHealthcheck() {
  if (!isPostgresEnabled() && !isDualWriteEnabled()) return { ok: false, reason: 'disabled' };
  try {
