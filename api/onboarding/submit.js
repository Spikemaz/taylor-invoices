...
// Central Drive folder (operator-owned) — service accounts have 0 Drive
// quota of their own, so every file/folder we create MUST be parented inside
// this folder so storage counts against the operator's human Google account.
const CENTRAL_DRIVE_FOLDER_ID = process.env.GOOGLE_DRIVE_FOLDER_ID;
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // All user data lives inside the operator's central Drive folder, owned by
    // the human operator (so storage counts against their quota, not the
    // service account's zero quota). Users access their data via the web app
    // only — sheets/folders are NOT shared with their personal Gmail.

    // --- Per-user spreadsheet (inside central hub) ---
    const userSheet = await drive.files.create({
      requestBody: {
        name: `BooksIQ - ${user.name} - ${userId.slice(0, 8)}`,
        mimeType: 'application/vnd.google-apps.spreadsheet',
        parents: [CENTRAL_DRIVE_FOLDER_ID],
      },
      fields: 'id',
    ...
    // --- Per-user invoice folder (inside central hub) ---
    const userFolder = await drive.files.create({
      requestBody: {
        name: `Invoices - ${user.name} - ${userId.slice(0, 8)}`,
    ...
    // Set up the user sheet with all required tabs and initial data
    await setupUserSheet(sheets, userSheetId, { user, entityType, selfEmployed, ltdCompany, practices });
    console.log(`[Onboarding] Initialized sheet with tabs and data`);
    ...
    const userRow = [
      ...
      userSheetId,                      // L: backupSheetId (same as F under central-hub architecture)
      userFolderId,                     // M: backupFolderId (same as G under central-hub architecture)
