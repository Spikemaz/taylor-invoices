...
async function updateInvoice(sheets, sheetId, { num, updates, practice, practiceAlts }, res, backupSheetId = null) {
  ...
  if (rowIndex === -1) {
    // Distinguish "this invoice number does not exist at all" (absent — the
    // row was deleted; client may safely self-heal its local ghost copy) from
    // "same-num rows exist but none match the practice" (ambiguous — do NOT
    // let the client treat a real invoice as a ghost).
    const wantNum = String(num);
    const numExists = rows.some(row => row[0] === num || String(row[0] || '') === wantNum);
    return res.status(404).json({ error: 'Invoice not found', num, reason: numExists ? 'ambiguous' : 'absent' });
  }
...
const SHARED_SETTINGS_KEYS = ['dayMap', 'ahPrac', 'payTerms', 'invoiceFooter', 'entities', 'defComm', 'customHolidays', 'scheduleExceptions', 'workSchedule', 'scheduleHours', 'draftServices', 'serviceDurations', 'practiceStartDates'];
