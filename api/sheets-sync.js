...
async function deleteInvoice(sheets, sheetId, { num, driveLink }, res, userId, backupSheetId = null, backupFolderId = null) {
  ...
  return res.status(200).json({
    success: true,
    message: `Invoice #${num} moved to Trash${hasPdf ? (pdfCleanup ? (pdfCleanup.requiresMigration ? ' (PDF cleanup deferred — account migration pending)' : ' (PDF trashed server-side)') : ' (PDF will be moved by Apps Script)') : ''}`,
    num,
    hasPdf,
