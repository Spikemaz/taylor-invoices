...
// Extract the Drive file ID from a webViewLink-style URL.
...
function extractDriveFileId(link) {
  ...
  if (m) return m[1];
  // Bare ID (no protocol/punctuation)
...
// Mirror lookup MUST use the same year/month derivation as the upload
// path (frontend's parsePeriodForFolder + drive-upload.js), otherwise an
// invoice whose `period` month differs from its `date` month (common —
// February invoices billed on 1 March, etc.) won't be found in the
// backup tree and the mirror copy stays orphaned after delete. Order:
// invoice.period (preferred, matches upload) → invoice.date (fallback).
...
// Reconstruct the path drive-upload.js wrote the mirror to and look the
// file up by name. We re-derive Entity/Invoices/Year/Month[/Ad Hoc] from
// the invoice row rather than store a mirror file ID, because adding a
// schema column would invalidate every legacy invoice.
...
// generate-pdf.js builds: <PREFIX> Invoice-<num>.pdf where prefix is HTCS
// for ltd, TAYLOR otherwise. Stay aligned with that for mirror lookup.
...
async function deleteInvoice(sheets, sheetId, { num, driveLink }, res, userId, backupSheetId = null, backupFolderId = null) {
  const response = await sheets.spreadsheets.values.get({
    spreadsheetId: sheetId,
    range: 'Invoices!A:AD',
  });
  ...
  const cleanupNote = (() => {
    if (!hasPdf) return '';
    if (pdfCleanup) {
      return `, PDF ${pdfCleanup.primaryTrashed ? 'trashed server-side' : 'trash failed'}` +
        (pdfCleanup.mirrorAttempted ? ` (mirror ${pdfCleanup.mirrorTrashed ? 'trashed' : 'not trashed'})` : '');
    }
  ...
  return res.status(200).json({
    success: true,
    message: `Invoice #${num} moved to Trash${hasPdf ? (pdfCleanup ? ' (PDF trashed server-side)' : ' (PDF will be moved by Apps Script)') : ''}`,
    num,
    hasPdf,
...
async function triggerPdfRegeneration(sheets, sheetId, { num }, res, backupFolderId = null) {
  const response = await sheets.spreadsheets.values.get({
    spreadsheetId: sheetId,
    range: 'Invoices!A:AD',
  });
