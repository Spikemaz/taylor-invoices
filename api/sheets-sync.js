...
// Find an invoice row by num, optionally also matching its practice column so we
// can disambiguate when two practices share an invoice number on the same sheet.
// `rows` is the raw .values array (no header row stripped). Index 2 of each row
// corresponds to INVOICE_COLUMNS[2] === 'practice'.
function matchInvoiceRowIndex(rows, num, practice) {
  ...
  if (practice !== undefined && practice !== null && String(practice).trim() !== '') {
    const wantPrac = String(practice).trim().toLowerCase();
    // 1) Exact practice match wins.
    const exact = numMatches.find(i =>
      String(rows[i][2] || '').trim().toLowerCase() === wantPrac);
    if (exact !== undefined) return exact;
    // 2) Legacy fallback: only allow plain-num match when there is no
    //    ambiguity — either a single num match exists, or every matching
    //    row has a blank practice column. Otherwise refuse to guess.
    const allBlank = numMatches.every(i =>
      String(rows[i][2] || '').trim() === '');
    if (numMatches.length === 1 || allBlank) return numMatches[0];
    return -1;
  }
...
async function deleteInvoice(sheets, sheetId, { num, driveLink, practice }, res, userId, backupSheetId = null, backupFolderId = null) {
  const response = await sheets.spreadsheets.values.get({
    spreadsheetId: sheetId,
    range: 'Invoices!A:AD',
  });
  ...
  const rowIndex = matchInvoiceRowIndex(rows, num, practice);
  ...
  if (backupSheetId) {
    try {
      ...
      const backupRows = backupResponse.data.values || [];
      const backupRowIndex = matchInvoiceRowIndex(backupRows, num, practice);
      if (backupRowIndex !== -1) {
        const backupMetadata = await sheets.spreadsheets.get({ spreadsheetId: backupSheetId });
...
async function updateInvoice(sheets, sheetId, { num, updates, practice }, res, backupSheetId = null) {
  const response = await sheets.spreadsheets.values.get({
    spreadsheetId: sheetId,
    range: 'Invoices!A:AD',
  });
...
async function updateInvoiceStatus(sheets, sheetId, { num, paidStatus, paidDate, practice }, res, backupSheetId = null) {
  try {
    const response = await sheets.spreadsheets.values.get({
      spreadsheetId: sheetId,
      range: 'Invoices!A:AE',
    ...
    const dataIdx = matchInvoiceRowIndex(dataRows, num, practice);
    ...
    if (backupSheetId) {
      try {
        ...
        const backupDataIdx = matchInvoiceRowIndex(backupDataRows, num, practice);
...
async function triggerPdfRegeneration(sheets, sheetId, { num, practice }, res, backupFolderId = null) {
  const response = await sheets.spreadsheets.values.get({
    spreadsheetId: sheetId,
    range: 'Invoices!A:AD',
  });
