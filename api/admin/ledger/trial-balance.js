/**
 * GET /api/admin/ledger/trial-balance?entityId=...&asOf=YYYY-MM-DD
 *
 * Read-only trial balance for an entity. Admin-only. Stage 1 surface.
 */

const { requireSession, applyCors } = require('../../_lib/auth');
const { isPostgresEnabled, isDualWriteEnabled } = require('../../_lib/db');
const { trialBalance } = require('../../_lib/ledger/reports');
...
module.exports = async function handler(req, res) {
  ...
  try {
    const tb = await trialBalance(entityId, asOf);
    return res.status(200).json({ ok: true, entityId, ...tb });
  } catch (err) {
    console.error('[admin/ledger/trial-balance] failed:', err);
