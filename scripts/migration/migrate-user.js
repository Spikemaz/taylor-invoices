...
const {
  ...
  createSpreadsheet, getOrCreateSpreadsheet, getOrCreateFolder, shareWithUser,
...
async function main() {
  ...
  // Use getOrCreateSpreadsheet (not createSpreadsheet) so a re-run after
  // a crash between drive.files.create and saveState doesn't leave a
  // duplicate sheet behind — we look up by exact name+parent and reuse.
  const newSheetId = await runStep(state, statePath, 'create_user_facing_sheet', async () => {
    const id = await getOrCreateSpreadsheet(drive, newSheetName, centralFolderId);
    await ensureSheetSchema(sheets, id);
    log('info', `User-facing sheet id=${id}`);
    return id;
  });
  ...
  const backupSheetId = await runStep(state, statePath, 'create_hidden_backup_sheet', async () => {
    const id = await getOrCreateSpreadsheet(drive, backupSheetName, hiddenBackupRootId);
    await ensureSheetSchema(sheets, id);
    log('info', `Hidden backup sheet id=${id}`);
    return id;
  });
